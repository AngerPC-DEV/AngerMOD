...
local function MakeTab(text)
    local b = Instance.new("TextButton", TabFrame); b.Size=UDim2.new(0.31,0,1,0); b.BackgroundColor3=Color3.fromRGB(20,20,20); b.Text=text; b.TextColor3=Color3.new(1,1,1); b.Font=Enum.Font.SciFi; style(b)
    return b
end
...
btnTabMain.MouseButton1Click:Connect(function() PageMain.Visible=true; PageInfo.Visible=false; PageUI.Visible=false end)
btnTabInfo.MouseButton1Click:Connect(function() PageMain.Visible=false; PageInfo.Visible=true; PageUI.Visible=false end)
btnTabUI.MouseButton1Click:Connect(function() PageMain.Visible=false; PageInfo.Visible=false; PageUI.Visible=true end)
...
RunService.RenderStepped:Connect(function()
    ...
    if States.Aim then
        ...
        if target then 
            ...
            local lookAtPos = target.Position + Vector3.new(shakeX, shakeY, 0)
            Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, lookAtPos), valSmooth) 
        end
    end
        ...
        if tool and tool:FindFirstChild("Handle") then
            for _, v in pairs(game.Players:GetPlayers()) do
                if v ~= Player and v.Character and v.Character:FindFirstChild("Head") and v.Character.Humanoid.Health > 0 then
                    ...
                    if dist < 50 then tool.Handle.CFrame = v.Character.Head.CFrame; tool:Activate(); pcall(function() firetouchinterest(tool.Handle, v.Character.Head, 0); firetouchinterest(tool.Handle, v.Character.Head, 1) end); break end
                end
            end
        end
    end
