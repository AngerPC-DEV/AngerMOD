...
local RecordedPath = {}
local UI_Unlocked = false -- Global state for UI editing
...
-- TITLE
local Title = Instance.new("TextLabel", Main); Title.Size=UDim2.new(1,0,0,45); Title.BackgroundTransparency=1; Title.Text="AngerPC V124 (FIXED)"; Title.Font=Enum.Font.SciFi; Title.TextSize=24; Title.TextColor3=Color3.new(1,1,1); table.insert(RGB_Objects, {Type="Text", Instance=Title})
...
local function makeBind(name, callback)
    local hb = Instance.new("TextButton", ScreenGui)
    hb.Name="Bind_"..name
    hb.Size=UDim2.new(0,50,0,50)
    hb.Position=UDim2.new(0.85,0,0.4,0)
    hb.BackgroundColor3=Color3.fromRGB(15,15,15)
    hb.Text=name:sub(1,3)
    hb.TextColor3=Color3.new(1,1,1)
    hb.Visible=false
    
    -- Sync Draggable State Immediately
    hb.Active = UI_Unlocked 
    hb.Draggable = UI_Unlocked
    
    style(hb,25)
    hb.MouseButton1Click:Connect(callback)
    table.insert(Movable_Objects, hb)
    return hb
end
...
addOption("INF ZOOM", "UnlockAll", false) -- FIXED
...
UnlockBtn.MouseButton1Click:Connect(function()
    UI_Unlocked = not UI_Unlocked -- Global Toggle
    ...
    for _, obj in pairs(Movable_Objects) do 
        obj.Active = UI_Unlocked 
        obj.Draggable = UI_Unlocked 
    end
end)

local ConfigName = "AngerConfig_V124.json"
