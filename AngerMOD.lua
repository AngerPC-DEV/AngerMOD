...
local ChatHistory = {
    {
        role = "system",
        content = "Ты — AngerPC, крутой ИИ-бот в Roblox. Твой создатель — AngerPC-DEV. Твой характер: дерзкий, веселый. Отвечай кратко."
    }
}
...
ScreenGui.ResetOnSpawn = false -- !!! ГЛАВНЫЙ ФИКС: МЕНЮ НЕ ПРОПАДАЕТ ПРИ СМЕРТИ !!!
...
-- TITLE
local Title = Instance.new("TextLabel", Main); Title.Size=UDim2.new(1,0,0,45); Title.BackgroundTransparency=1; Title.Text="AngerPC V122 (NO DEATH RESET)"; Title.Font=Enum.Font.SciFi; Title.TextSize=24; Title.TextColor3=Color3.new(1,1,1); table.insert(RGB_Objects, {Type="Text", Instance=Title})
...
local function addOption(name, key, useInput, defaultInputVal, inputCallback)
    ...
    local function Toggle()
        ...
        -- Логика для Instant Cam
        if key == "InstantCam" then
            if States.InstantCam then Camera.CameraType = Enum.CameraType.Scriptable else Camera.CameraType = Enum.CameraType.Custom end
        end
    end
...
-- [ OPTIONS ] --
addOption("INSTANT CAM", "InstantCam", false) -- !!! НОВАЯ ФУНКЦИЯ !!!
...
Player.CharacterAdded:Connect(function(char) 
    ...
    -- Сброс камеры при респавне, чтобы не застрять
    if States.InstantCam then Camera.CameraType = Enum.CameraType.Custom end
end)
